...
void replace_page_lru(int pid, int swap_page_number, int logical_page_number) {
    auto lru = *lru_set.begin();
    int lru_page_number = lru.second;
    lru_set.erase(lru_set.begin());

    Page* lru_page = main_memory[lru_page_number];
    int remove_pid = lru_page->pid;
    int remove_logical_page = lru_page->logical_page_number;
    Page* desired_page = swap_memory[swap_page_number];
    
    free_swap_pages.erase(swap_page_number);
    swap_memory[swap_page_number] = lru_page;
    pid_to_page_map[remove_pid][remove_logical_page] = make_pair(swap_page_number, 0);
    
    main_memory[lru_page_number] = desired_page;
    main_memory[lru_page_number]->tou = global_time_counter++;
    pid_to_page_map[pid][logical_page_number] = make_pair(lru_page_number, 1);
    
    lru_set.insert({main_memory[lru_page_number]->tou, lru_page_number});
}
...
int* resolve_address(int pid, int logical_address) {
    ...
    if (flag == 1) {
        lru_set.erase(make_pair(main_memory[physical_page]->tou, physical_page));
        main_memory[physical_page]->tou = global_time_counter++;
        lru_set.insert(make_pair(main_memory[physical_page]->tou, physical_page));
        ...
        if (!free_main_pages.empty()) {
            ...
            lru_set.erase({main_memory[physical_page_number]->tou, physical_page_number});
            ...
            lru_set.insert({global_time_counter-1, physical_page_number});
            return &main_memory[pid_to_page_map[pid][page_number].first]->content[offset];
        }
        else{
        replace_page_lru(pid, physical_page, page_number);
        return &main_memory[pid_to_page_map[pid][page_number].first]->content[offset];
        }
...
void load_executable(const string& filename, int& pid_counter) {
    ...
    for (int i = 0; i < required_pages; i++) {
        ...
        if (physical_page_number != -1 && flag == 1) {
            ...
            lru_set.insert({main_memory[physical_page_number]->tou, physical_page_number});
        } else if (physical_page_number != -1) {
            swap_memory[physical_page_number]->pid = pid;
            swap_memory[physical_page_number]->tou = global_time_counter++;
...
void kill_process(int pid) {
    ...
    for (int i = 0; i < page_table.size(); i++) {
        ...
        if (flag == 1) free_main_pages.insert(physical_page);
...
void run_process(int pid) {
    ...
    while (getline(infile, command)) {
        ...
        if (operation == "load") {
            int value, logical_address;char sep;
...
int main(int argc, char* argv[]) {
    ...
    for (int i = 0; i < MAIN_MEMORY_PAGES; ++i) 
    {
      ...
      lru_set.insert({main_memory[i]->tou, i});
